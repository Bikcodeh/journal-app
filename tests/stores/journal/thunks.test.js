...
import { startAddNewNote, startDeletingNote, startLoadingNotes, startSavingNote, startUploadingFiles } from "../../../src/store/journal/thunks";
...
describe('Tests for Journal Thunks', () => {
    ...
    test('startDeletingNote should call deleteNoteById', async () => {
        ...
        expect(dispatch).toHaveBeenCalledWith({ "payload": undefined, "type": "journal/deleteNoteById" });
    });
    ...
    test('startUploadingFiles should call setImagesToActiveNote', async () => {
        ...
        expect(dispatch).toHaveBeenCalledWith({"payload": [], "type": "journal/setImagesToActiveNote"})
    });
});